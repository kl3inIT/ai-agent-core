 ...
 *
 * <ol>
 *   <li><b>perRequestAssemblyIncludesBuiltInsAndHostContributor</b> — asserts {@link
 *       AgentToolCallbacks#forCurrentUser()} composes the six built-in @Tool methods with the
 *       host's {@link OrderSummaryToolContributor} (SPI-01).
 *   <li><b>findRecordsOrderRoundTrip</b> — seeds a Customer + Order under {@link
 *       SystemAuthenticator} and asserts {@code BuiltInDataTools.findRecords("jmixapp_Order", null,
 *       null)} returns the seeded row's number via the real {@link DataManager} path (success
 *       criterion #3 DataManager round-trip).
 *   <li><b>describeEntityAdminPathSurfacesStructuredJson</b> — asserts admin-user {@code
 *       describeEntity} returns structured JSON that names the entity and at least one expected
 *       attribute (smoke check that describe_entity plumbing works end-to-end).
 * </ol>
 *
 * <p><b>Restricted-user denied-attribute absence</b> — success criterion #3's second half — is
 * covered authoritatively at unit level by Plan 04's {@code CurrentUserSchemaAccessTest} (see
 * 03-CONTEXT.md `## Deferred Ideas`). This class intentionally does NOT add a restricted-user
 * {@code @SpringBootTest} variant.
 *
 * <p><b>ChatService-routed mock-ChatModel variant</b> is deferred to Phase 4 alongside {@code
 * ChatClientFactory} + advisor chain wiring (CHAT-02/CHAT-03). Here we call the {@link
 * BuiltInDataTools} @Tool method directly — that is the authoritative Phase 3 coverage of the
 * DataManager path.
 */
@SpringBootTest
...
class ChatServiceToolIntegrationTest {
  ...
  void perRequestAssemblyIncludesBuiltInsAndHostContributor() {
    ...
    Set<String> names =
        Arrays.stream(cbs).map(cb -> cb.getToolDefinition().name()).collect(Collectors.toSet());
    assertThat(names)
        .contains(
            "list_entities",
            "describe_entity",
  ...
  void findRecordsCustomerByVietnameseNameContains() {
    ...
    String json =
        builtInDataTools.findRecords(
            "Customer", new LeafNode("name", "CONTAINS", customerName), null);
  ...
  void findRecordsToolCallbackFindsCustomerByVietnameseNameContains() {
    ...
    String unrelatedCustomerName = "Không Khớp Callback " + System.currentTimeMillis();
    systemAuthenticator.runWithSystem(
        () -> {
          Customer unrelatedCustomer = metadata.create(Customer.class);
          unrelatedCustomer.setName(unrelatedCustomerName);
          dataManager.save(unrelatedCustomer);
    ...
    ToolCallback findRecords =
        Arrays.stream(agentToolCallbacks.forCurrentUser())
    ...
    assertThat(json)
        .as(
            "find_records callback should bind structured filter object and find Vietnamese names; raw=%s",
            json)
        .contains(customerName)
        .doesNotContain(unrelatedCustomerName)
        .contains("\"truncated\":false");
  }

  @Test
  void findRecordsToolCallbackAllowsMissingFilter() {
    String customerName = "No Filter Customer " + System.currentTimeMillis();
    systemAuthenticator.runWithSystem(
        () -> {
          Customer customer = metadata.create(Customer.class);
          customer.setName(customerName);
    ...
    ToolCallback findRecords =
        Arrays.stream(agentToolCallbacks.forCurrentUser())
            ...
            .orElseThrow();
    String input =
        objectMapper.createObjectNode().put("entityName", "Customer").put("limit", 100).toString();

    String json = unwrapToolCallbackStringResult(findRecords.call(input));

    assertThat(json)
        .as("find_records callback should allow omitted filter and list readable rows; raw=%s", json)
        .doesNotContain("invalid_filter")
        .contains(customerName)
        .contains("\"truncated\":false");
  }
  ...
  void findRecordsToolCallbackAcceptsStringifiedFilter() {
    ...
    String unrelatedCustomerName = "Không Khớp String Filter " + System.currentTimeMillis();
    systemAuthenticator.runWithSystem(
        () -> {
          Customer unrelatedCustomer = metadata.create(Customer.class);
          unrelatedCustomer.setName(unrelatedCustomerName);
