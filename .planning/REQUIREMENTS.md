...
- [ ] **TEST-04**: Security negative-case suite: user without read access to an entity receives filtered schema AND execution is denied; RAG retrieval filters out forbidden roles; cross-user conversation access denied *(Phase 08-01: 6 @Test methods landed across FilteredSchemaAndExecutionDenialTest + RagRoleFilterNegativeTest + CrossUserConversationAccessTest; 3 PASS / 3 RED — REDs reveal real SUT gaps that trigger R-XP-2 `--gaps` replan; full GREEN gate held until SUT fixes land in Wave 2)*
