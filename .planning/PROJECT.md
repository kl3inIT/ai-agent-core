...
- [ ] `AiAgentAdminRole` gates Parameters / KB / Audit / Exposure views
...
- [ ] Entity exposure policy — programmatic control of which `MetaClass`/`MetaProperty` the agent sees
...
**Testing**
- [ ] Unit tests for metamodel scanner, schema generator, tool generator, exposure policy, audit persistence
...
- **Security**: Jmix roles + data security is the primary enforcement layer; AI exposure layer is additive, never a replacement
