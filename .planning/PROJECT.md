...
**Shipped versions:** v1.0.0 MVP (2026-04-26) · v1.1.0 Prompt Hardening, Mutation Tools & Configurable Chat Surfaces (2026-05-11, via PR #28) · v1.2.0 Operator Experience & Runtime Performance (2026-06-12).

v1.1.0 turned the read-only MVP into a mutation-capable, governance-aware, multi-surface AI agent without new core dependencies: prompt/tool-contract hardening (`agent.entities`/`agent.permissions` baseline, leak guards, `unknown_entity` retry), an admin LLM-exposure denylist (`AiExposureRule` + `LlmExposurePolicy`, uniform `unknown_entity` opacity), opt-in built-in mutation tools (layered fail-closed gating, idempotency, PII-safe errors, full audit), configurable chat surfaces (`FULL_ROUTE` + `HEADER_BUTTON` over one `ChatPanelFragment`, cross-surface continuity), chat task files (transient, attach + multimodal `Media` read + `bulk_save_records`, default model swap to Apache-2.0 `qwen/qwen3.6-35b-a3b`, CRM-style right-pane with per-turn-all injection), and intent-driven extraction → prefilled Jmix forms (`IntentExtractor<T>` SPI, `prepare_form_draft` / `propose_action_choices`, controller-side-only navigation, permission-gated prefill).

v1.2.0 made the agent tunable and observable — still zero new core dependencies. It shipped four near-independent feature areas: a third `SIDEBAR` chat surface over the same `ChatPanelFragment` with in-chat observability (ephemeral KIND-keyed streaming-status line + collapsed per-turn tool-detail disclosure, no new persisted "turn" entity, UI-layer leak test); an admin model picker (curated open-weights `ComboBox` + custom escape hatch, admin-only, validity checked at first use → per-request `ChatOptions`) plus a documented three-tier config-knob taxonomy (Tier-1 runtime-editable / Tier-2 boot read-only / Tier-3 secret indicator-only) with an `AiSettingsChangedEvent` eviction hook; mutation-internals hardening (canonical `MutationGateChain`, constrained batch FK loads, memoized related-write metadata, byte-for-byte v1.1 parity); and a targeted AI-runtime performance pass (per-turn `RunContext` memoization, app-wide denylist cache evicted on `LlmExposureChangedEvent`, RAG `Filter.Expression` once per retrieval, task-file `Media` regression-locked — every change with a checkable proxy, 865 tests green). **Voice Input was descoped from v1.2.0** (Phase 19 never started → Backlog 999.2); the milestone was renamed to drop "Voice Input".
...
- Nyquist `*-VALIDATION.md` exists only for Phase 14; backfill phases 9/10/11/12/13/13.1 with `/gsd-validate-phase`.
...
- Open at v1.2.0 close: 2 debug sessions (`bulk-create-allowlist-collision` [fixing], `bulk-create-confirm-throws` [awaiting_human_verify]); Phase 17 UAT 4 pending scenarios (non-blocking — MUT-18 parity HOLDS via the automated suite). See `STATE.md` § Deferred Items.

## Next Milestone Goals (v1.3 — to be defined)

Candidate scope for the next milestone (define with `/gsd-new-milestone`):

- **Chat voice input — Soniox STT (+ OpenAI fallback)** — Backlog 999.2, the descoped v1.2 Phase 19 with full scope preserved (STT-01..06, TEST-18).
- **Admin-rotated provider credentials (API key + base URL)** — Backlog 999.1; in-app credential rotation without redeploy, within the SEC-08 secret-redaction invariants.
- **Carried hardening debt** — Phase 10 re-verification + Nyquist `*-VALIDATION.md` backfill (phases 9/10/11/12/13/13.1); clean-consumer smoke (PKG-05 / TEST-07).

**Deferred to a later hardening pass:** `TranscriptionPostProcessor` + custom STT-provider SPIs; per-conversation end-user model switching; admin-screen performance work; attribute-path-level exposure rules; dormant-seed activation (SEED-001/002/003/004/006/008/009 triggers not met).
...
- ✓ AI-runtime performance pass (targeted): per-turn memoization anchored to one `RunContext` ThreadLocal slot (active-turn-gated safe-miss, wiped in `clear()`) routing `LlmExposurePolicy` CRUD verdicts + readable schema; app-wide denylist `ConcurrentHashMap`+`computeIfAbsent` (immutable view) evicted on `@EventListener(LlmExposureChangedEvent)`; RAG `Filter.Expression` built once per retrieval (clauses verbatim); task-file `Media` encode regression-locked once-per-`(conversationId, taskFileId)`-per-turn (D-10 proxy-first, no cache added); one checkable proxy per optimization; `ToolQueryCountBaselineTest` SELECT ceiling lowered (line 151 only); no benchmark harness / Caffeine / admin-screen perf / new component; all existing security/exposure/audit/tool/RAG suites unchanged (865 tests green) — v1.2.0 / Phase 18 (PERF-01..05)
- ✓ Right-sidebar chat surface & observability UX: `SIDEBAR` third surface over the same `ChatPanelFragment` (no second chat backend/memory, `AiUiSettings` participation, `AiChatSessionState` continuity); ephemeral KIND-keyed streaming-status line in a sibling slot (clears on finalization, never leaks `@Tool`/entity names); collapsed-by-default per-turn tool-detail disclosure (humanized label-only steps + timing, hidden when empty); driven by the existing `StreamingEvent` flux + `AiAuditEvent` tree — no new persisted "turn" entity; `ObservabilityLeakTest` reuses the Phase 9 leak packs at the UI layer — v1.2.0 / Phase 15 (SURF-11, OBS-01, OBS-02, OBS-04, TEST-19)
- ✓ Admin model picker + config-knob migration: curated open-weights `ComboBox` (default marked) + custom free-entry escape hatch, admin-only, validity checked at first use (graceful fallback) → per-request `ChatOptions`; documented three-tier knob taxonomy (Tier-1 runtime-editable via `AiUiSettingsResolver` read-through / Tier-2 boot read-only "requires restart" / Tier-3 secret "configured: yes/no" only); `AiSettingsChangedEvent` single-publish-site eviction hook; secrets never editable/displayed (SEC-08); curated-allowlist test (TEST-20) — v1.2.0 / Phase 16 (MODEL-01..03, CFG-01..03, SEC-08, TEST-20)
- ✓ Mutation-internals hardening: canonical `MutationGateChain` `@Component` (five mutation tools as thin adapters; `@Transactional` only on `MutationSaveExecutor.save`; source-level gate-order invariant), constrained batch FK loads (one `.ids()` per target class, row-level security preserved), memoized related-write metadata; byte-for-byte v1.1 parity — Phase 9/10/11 mutation suites pass unchanged (MUT-18 HOLDS) — v1.2.0 / Phase 17 (MUT-15..18)

### Active

No active milestone. v1.2.0 shipped 2026-06-12; define the next milestone with `/gsd-new-milestone`. Top candidates carried in **Deferred** below: Chat Voice Input (Backlog 999.2), admin-rotated provider credentials (Backlog 999.1), and the carried hardening debt.

### Deferred (carried to v1.3+ / later hardening pass)

- [ ] **Chat voice input — Soniox STT (+ OpenAI fallback)** — descoped from v1.2.0 (Phase 19 never started) → ROADMAP Backlog Phase 999.2, full scope preserved (STT-01..06, TEST-18): Soniox default + OpenAI-direct fallback, `MediaRecorder` capture, privacy-safe `STT_TRANSCRIPTION` audit (hash default / raw opt-in), non-blocking error + retry UI, transcript → `MessageInput` for review before send. (`TranscriptionPostProcessor`/custom-provider SPIs still deferred.)
- [ ] **Admin-rotated provider credentials (API key + base URL)** — ROADMAP Backlog Phase 999.1; in-app credential rotation without redeploy, within the SEC-08 secret-redaction invariants.
...
| **v1.2**: `SIDEBAR` is a third surface over the SAME `ChatPanelFragment`; in-chat observability is driven by the existing `StreamingEvent` flux + `AiAuditEvent` tree — no second chat backend, no new persisted "turn" entity | Cross-surface continuity + zero duplication + no parallel state store | ✓ Good |
| **v1.2**: Three-tier config-knob taxonomy (Tier-1 runtime-editable / Tier-2 boot read-only / Tier-3 secret indicator-only); secrets never editable or displayed; model validity checked at first use, not at save time | SEC-08 fail-closed; avoids blocking admin saves on transient provider reachability | ✓ Good |
| **v1.2**: One canonical `MutationGateChain` with `@Transactional` only on `MutationSaveExecutor.save`; every gate throws before the save | Fail-closed byte-for-byte parity with v1.1 (MUT-18); kills the duplicated gate sequence before the perf pass | ✓ Good |
| **v1.2**: Perf caches wire to an invalidation hook (`LlmExposureChangedEvent` for exposure-derived, `AiSettingsChangedEvent` for settings); user/role/exposure data stays per-`RunContext` (one turn); no benchmark harness / Caffeine / new component | Correctness over raw speed; every optimization ships a checkable proxy | ✓ Good |
| **v1.2**: Voice Input (Phase 19) descoped → Backlog 999.2; milestone renamed to drop "Voice Input" | Ship the four completed feature areas; STT never started — don't hold the milestone for unstarted scope | — Pending (carried to v1.3) |
...
---
*Last updated: 2026-06-12 after v1.2.0 milestone (Operator Experience & Runtime Performance) — Phases 15–18 shipped; Voice Input (Phase 19) descoped → Backlog 999.2*
