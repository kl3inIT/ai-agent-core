...
| **Quick run command** | `cd ai-agent && ./gradlew :ai-agent:ai-agent:test --tests "com.vn.agent.<Class>"` |
| **Full suite command** | `cd ai-agent && ./gradlew :ai-agent:ai-agent:test` (excludes `live`, `rag-it`, `eval`) |
...
- **After every task commit:** Run the single most-relevant `cd ai-agent && ./gradlew :ai-agent:ai-agent:test --tests "<Class>"` (each <30s under the pure-JUnit/Mockito style)
- **After every plan wave:** Run `cd ai-agent && ./gradlew :ai-agent:ai-agent:test` (full default suite)
...
| 18-02-01 | 18-02 | 2 | PERF-01 | T-18-04, T-18-05 | one `RunContext` per-turn cache slot added; safe-miss accessor on foreign threads; wiped in `clear()`; no process-wide runId map, no reactor-Context propagation | source assertion (compile + grep) | `cd ai-agent && ./gradlew :ai-agent:ai-agent:compileJava` | ❌ W0 | ⬜ pending |
| 18-02-02 | 18-02 | 2 | PERF-01 + boundary (D-09) | T-18-04, T-18-06, T-18-07 | CRUD verdicts + readable schema resolved once per `RunContext` across N tool calls; cache empty after `RunContext.clear()`; per-turn cache symbol confined to `LlmExposurePolicy` + `RunContext`, absent from `BuiltInDataTools` | call-count (Mockito) + ThreadLocal-empty assert + source-scan boundary invariant | `--tests "com.vn.agent.exposure.LlmExposurePerTurnMemoTest" --tests "com.vn.agent.exposure.PerTurnCacheBoundaryInvariantTest"` | ❌ W0 (template: `RelatedWriteMetadataMemoTest`) | ⬜ pending |
...
| 18-04-01 | 18-04 | 1 | PERF-04 | T-18-11, T-18-12, T-18-13 | per-file encode (FileStorage read + Tika) + `resolveTaskFile*` ≤ once per `(convId, taskFileId)` per turn (regression-lock) or memoized+evicted (cache branch); row load stays constrained `DataManager` | call-count on FileStorage read + Tika + `resolveTaskFile*` (branch per D-10) | `--tests "com.vn.agent.taskfile.TaskFileMediaEncodeOncePerTurnTest" --tests "com.vn.agent.taskfile.PerTurnMediaInjectionTest"` | ✅ PerTurn / ❌ W0 encode call-count | ⬜ pending |
