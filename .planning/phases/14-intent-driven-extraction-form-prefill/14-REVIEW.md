...
> **Fix pass 2026-05-11:** WR-01..WR-06 are all RESOLVED on branch `gsd/phase-14-intent-driven-extraction-form-prefill` (commits `686fb4b` WR-03, `7729ce4` WR-01, `5f6be7e` WR-02, `bc0928a` WR-06, `657a1f4` WR-05, `616b8b6` WR-04). INFO items IN-01..IN-05 remain open.

### WR-01: `OpenFormWithDraftHandler.afterNavigation` does not handle exceptions from `draftLoader.apply(...)` — RESOLVED (`7729ce4`)
...
### WR-02: Denied/skipped prefill attributes are silently dropped with no user feedback — RESOLVED (`5f6be7e`)
...
### WR-03: `ExtractionService.prepare` can persist a draft and then return a failure to the LLM — RESOLVED (`686fb4b`)
...
### WR-04: `looksLikeActionCancellation` matches substrings that are not cancellations — RESOLVED (`616b8b6`)
...
### WR-05: Stale action-choice row leaks when the model re-emits a proposal with a colliding id — RESOLVED (`657a1f4`)
...
### WR-06: `confirmAndDeleteDraft` performs save + remove in two separate transactions — RESOLVED (`bc0928a`)
