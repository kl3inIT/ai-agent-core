...
**Status:** Archived at milestone close on 2026-04-26
**Source:** .planning/REQUIREMENTS.md

## Outcome Notes

- 63/63 execution plans completed and summarized.
- Stale unchecked v1 boxes were normalized in this archive to reflect the shipped milestone state.
- Dropped/adjusted scope is preserved in the original text: SPI-04, SPI-08, UI-07, TEST-06, and AiExposureRule were removed/deferred by D-10.
- Known deferred debt remains explicit: PKG-05/TEST-07 clean-consumer smoke was investigated in Plan 08-05 and deferred because a minimal consumer needs PostgreSQL/pgvector or a starter stub VectorStore follow-up.
...
- [x] **PKG-01**: Add-on ships today as two Gradle modules under `ai-agent/`: `ai-agent` (functional) + `ai-agent-starter`. The `ai-agent-flowui` / `ai-agent-flowui-starter` split is deferred until a named REST-only consumer use case justifies it.
- [x] **PKG-02**: Active starter modules register auto-configuration via `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`
- [x] **PKG-03**: Add-on declares `@JmixModule(dependsOn = {CoreConfiguration.class, DataConfiguration.class, SecurityConfiguration.class, FlowuiConfiguration.class})` where appropriate for the current packaging shape
- [x] **PKG-04**: Zero-Vaadin functional-module posture is deferred with the 4-module split; current 2-module packaging keeps UI dependencies in `ai-agent-starter`
- [x] **PKG-05**: Clean-consumer smoke test: `publishToMavenLocal` + fresh Jmix project consumes `ai-agent-starter` and boots with default config

### Entities, Data & Security

- [x] **ENT-01**: Five Jmix JPA entities: `AiConversation`, `AiMessage`, `AiToolCallAudit`, `AiParameters`, `AiKnowledgeDocument` — UUID + `@JmixGeneratedValue` + `@Version` + `@InstanceName`, no Lombok. (`AiExposureRule` dropped per D-10 in Phase 02 CONTEXT; see MEMORY note "AI is just another Jmix client".)
- [x] **ENT-02**: All DDL owned by add-on Liquibase changelogs with `AI_AGENT_*` table-name prefix (no collisions with host)
- [x] **ENT-03**: Spring AI JDBC chat-memory DDL ported into add-on Liquibase (do not rely on `initialize-schema: true`)
- [x] **ENT-04**: pgvector extension created via Liquibase (`CREATE EXTENSION IF NOT EXISTS vector`) and distinct vector table name (e.g. `AI_AGENT_KB_VECTOR_STORE`)
- [x] **SEC-01**: Ship `AiAgentUserRole` (Chat view access) and `AiAgentAdminRole` (Parameters / KB / Audit views). (Exposure view dropped per D-10.)
- [x] **SEC-02**: Any authenticated Jmix user has Chat view by default; admin views gated to `AiAgentAdminRole`
- [x] **SEC-03**: All entity persistence via `DataManager` only — `EntityManager` forbidden in add-on code, enforced by code review + CLAUDE.md convention (ArchUnit deferred per D-10)
- [x] **SEC-04**: `AiConversation.createdBy` scoped to user; conversation replay and list filter by ownership at DataManager level

### Metadata-First Runtime & Tools

- [x] **TOOL-01**: `MetamodelScanner` reads Jmix `Metadata`/`MetaClass`/`MetaProperty` and produces raw inventory (cached once)
- [x] **TOOL-02**: Effective per-user schema computed per request via `AccessManager` directly — never cached per-app. (`EntityExposurePolicy` chain dropped per D-10; Jmix native security is authoritative.)
- [x] **TOOL-03**: Six generic read-only tools auto-generated: `list_entities`, `describe_entity`, `find_records`, `get_record`, `count_records`, `get_related_records`
- [x] **TOOL-04**: All tool bodies call `DataManager` (inheriting Jmix entity/attribute/row security); no native SQL, no JPQL authored by the LLM
...
- [x] **TOOL-06**: Hard row-count cap (default 20, max 100) on every collection-returning tool; LLM cannot override
- [x] **TOOL-07**: Tool result formatter wraps user-editable string fields in `<data>…</data>` delimiters with escaping to defuse prompt injection
...
- [x] **ORCH-06**: Default LLM provider: OpenAI-compatible via OpenRouter, configured through `spring.ai.openai.*` with `base-url` override; provider swappable by host replacing `ChatModel` bean

### Audit

- [x] **AUD-01**: `AuditAdvisor` writes `AiToolCallAudit` pre (tool invoked) and post (completed | failed | denied) entries
- [x] **AUD-02**: Audit persistence in `@Transactional(propagation = REQUIRES_NEW)` so tool rollback does not lose audit
- [x] **AUD-03**: Audit records include: conversationId, userId, tool name, input JSON, output summary, latency, outcome, denial reason
- [x] **AUD-04**: `AuditListener` SPI fires after each audit write (for Slack/SIEM/metrics side-channels); listener exceptions must not fail the main flow
- [x] **AUD-05**: Audit cannot be silently disabled — enforced by unit tests on `AuditAdvisor` + code review (ArchUnit deferred per D-10)
...
- [x] **PARAM-02**: Profile fields: model id, temperature, max tokens, system prompt, enabled tool names, RAG top-k, RAG similarity threshold
...
- [x] **SPI-01**: `ToolContributor` — hosts register additional `@Tool`-annotated beans
- [x] **SPI-02**: `ContextContributor` — inject per-request context (user, tenant, env) into prompt
- [x] **SPI-03**: `PromptContextContributor` — augment system prompt with host-specific instructions
- [x] **SPI-05**: `ToolGuard` — veto tool calls
- [x] **SPI-06**: `AuditListener` — observe audit writes for side-channels
...
- [x] **TEST-01**: Three-tier structure: `src/test` (unit), `src/integrationTest` (`@SpringBootTest` with mock `ChatModel`), `@Tag("live")` tests excluded from default `./gradlew test`
...
- [x] **TEST-06**: (removed per D-10 — ArchUnit rules deferred per MEMORY note "Avoid ArchUnit until drift"). Code review + the existing forbidden-import convention in CLAUDE.md remain authoritative until rule drift justifies ArchUnit.
- [x] **TEST-07**: Clean-consumer smoke: `publishToMavenLocal` → fresh minimal Jmix app consumes `ai-agent-starter` → boots + menu registers (runs in CI on release)
