...
**Phases:** 1-8, including inserted phases 7.1 and 7.2
**Total Plans:** 63

## Overview

Initial Jmix AI Copilot MVP: a reusable Jmix 2.8 add-on with metadata-first read tools, secured orchestration, RAG, guardrails, Flow UI, audit tree, release docs, and CI hardening.

## Milestone Summary

**Key accomplishments:**

- Established the two-module add-on/starter packaging and Maven-local consumer path.
- Built metadata-first, read-only DataManager tools governed by Jmix AccessManager/DataManager security.
- Shipped orchestration with Spring AI ChatClient, JDBC chat memory, conversation projection, durable audit events, and SPI hooks.
- Added pgvector-backed RAG ingestion/retrieval with role-scoped filters and guarded async status handling.
- Delivered Flow UI for chat, conversations, parameters, knowledge base, and audit tree.
- Closed release-readiness/security gaps with jmix-security-data, broad regression tests, GitHub Actions, operator docs, and 1.0.0 changelog.

**Issues deferred:**

- Clean-consumer smoke (PKG-05/TEST-07) remains deferred from Plan 08-05. The investigation found a real starter-consumability chain requiring PostgreSQL/pgvector or a stub VectorStore follow-up.
- Phase 8 Gap 3 / R-03e calibration and prompt-contract hardening bundle are tracked as future milestone work.
- 20 open planning artifacts were acknowledged as deferred at close; see .planning/STATE.md Deferred Items.
...
3. `publishToMavenLocal` + `jmix-app` in Maven-coord mode (composite `includeBuild 'ai-agent'` toggled off) boots with `ai-agent-starter` resolved from `~/.m2/repository`, per `docs/consumer-smoke.md` (D-02)
...
> Footnote (D-10): SPI-04 (`EntityExposurePolicy`), SPI-08 (per-SPI integration-test obligation), UI-07 (`ExposureRuleListView`), and TEST-06 (ArchUnit rules) were removed from v1. See `.planning/milestones/v1.0.0-phases/02-foundations/02-CONTEXT.md` §D-10 and `REQUIREMENTS.md` Scope Changes Log.
