...
must_haves:
  truths:
    ...
    - "Commit-state finalization, replay result construction, and non-throwing audit writes live in MutationCommitCoordinator; BuiltInMutationTools remains a thin orchestration bean and does not accumulate commit/audit helper bodies."
...
<task type="auto" tdd="false">
  ...
  <action>
1. Create/extract `MutationCommitCoordinator` and ensure all four tools use `MutationCommitState.NO_HOST_WRITE`, `HOST_SAVE_RETURNED`, and `INTENT_COMMITTED` through that collaborator.
...
<task type="auto" tdd="false">
  ...
  <action>
1. All mutation audit writes call `MutationCommitCoordinator.safeWriteAudit`, which catches/logs `RuntimeException` and never re-enters the tool catch ladder.
...
- `safeWriteAudit`, replay result construction, audit outcome mapping, and commit-unknown finalization live in `MutationCommitCoordinator`; `BuiltInMutationTools` contains only @Tool orchestration and collaborator calls for these concerns.
