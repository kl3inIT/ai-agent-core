...
public class ChatPanelFragment extends Fragment<VerticalLayout> {
    ...
    // ---- Streaming-status indicator (OBS-01) ------------------------------
     ...
     *  (the first-Content-implies-CHAT flip). Reset to {@code false} at turn start. */
    ...
    protected void onDetach(@NonNull DetachEvent detachEvent) {
        ...
        this.ownerUi = null;
        // Explicit observability-state teardown on detach. Element children go away with the
        // fragment; the field cleanup keeps the contract uniform.
        removeStatusRow();
    ...
    private void submitChatTurn(String displayText, String modelText,
        ...
        // Fresh per-turn observability state; neutral typing indicator until the first
        // Activity/Content arrives.
        turnContentSeen = false;
        ...
        activeStream = source
                ...
                .doOnNext(evt -> {
                    ...
                    // Ephemeral status line. The Activity variant is NOT routed through
                    // StreamEventRenderer (no markdown); flip the status and skip the markdown path.
    ...
    /**
     * 15-UAT Gap 1 — resolve the {@code AiConversation} id the server created for a run that
     * errored before delivering {@link StreamingEvent.Final}. {@link UnconstrainedDataManager}
     * + MANDATORY {@code where e.userUsername = :me and e.runId = :rid and e.parent is null}
     * filter; narrow projection (only {@code conversation.id}); {@code catch (RuntimeException)}
     * → debug-log + skip. Returns {@code null} when the run never reached the orchestration /
     * never created a conversation.
     */
    ...
    private void clearMessageList() {
        items.clear();
        actionChoiceRowsByProposalId.clear();
        // Drop observability state for the outgoing conversation; removeAllChildren() below
        // detaches the status <span>, but the fields must be nulled so the next turn re-creates
        // them in the fresh substrate.
        turnExtras.clear();
        statusRow = null;
        turnContentSeen = false;
        if (messageListSlot != null) {
