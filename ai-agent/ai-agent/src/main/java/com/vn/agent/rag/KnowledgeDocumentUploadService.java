...
/**
 * Creates {@link AiKnowledgeDocument} rows for user / SPI uploads and schedules the async ingestion
 * worker (RAG-06).
 *
 * <p><b>Role validation (D-07 fail-closed):</b> every submitted role code is resolved against
 * Jmix's {@link ResourceRoleRepository}; an unknown code aborts the save with {@link
 * UnknownRoleCodeException}. Empty {@code allowedRoles} is accepted (the document just becomes
 * invisible to non-admins per D-05 fail-closed retrieval) — validation only rejects codes that
 * exist but do not resolve to a role.
 *
 * <p><b>{@code sourceUri} contract:</b> the URI must be resolvable by Spring's {@code
 * ResourceLoader} — typically {@code classpath:} (for shipped / sample content) or {@code file:}
 * (for admin-uploaded content staged to a temp directory). RAG-01 requires PDF / MD / TXT / HTML
 * formats to work; the actual file-format dispatch lives in {@link AsyncIngestionWorker} which
 * delegates to Tika. Phase 7 Flow UI owns the stream-to-temp-file staging step; this service only
 * consumes already-resolvable URIs. Phase 2 {@code AiKnowledgeDocument} entity stores the URI in
 * the {@code fileName} column (see {@code AsyncIngestionWorker} source-resolution rules).
 *
 * <p><b>{@code sourceKind} contract:</b> a free-form label persisted into the {@code mimeType}
 * column — {@code application/pdf}, {@code text/markdown}, or synthetic labels like {@code
 * CUSTOM_INGESTER} are all acceptable. Phase 5 does not parse this field; it is metadata for the
 * admin UI and operators.
 *
 * <p><b>Async dispatch (Pitfall #7 avoidance):</b> the async worker is invoked only AFTER the
 * upload transaction commits. This uses {@link
 * TransactionSynchronizationManager#registerSynchronization} so the worker never observes a
 * document in PENDING state that has not yet been committed — a concurrent read could otherwise see
 * a phantom row.
 */
@Service
public class KnowledgeDocumentUploadService {
  ...
  /**
   * Validates role codes, persists a PENDING {@link AiKnowledgeDocument}, and schedules async
   * ingestion after commit.
   *
   * <p>Convenience overload that delegates to {@link #upload(String, String, Collection, String)}
   * with a {@code null} {@code sourceEntityName} (no entity link). Existing callers remain
   * source-compatible.
   *
   ...
   * @throws UnknownRoleCodeException if any supplied code is not defined in {@link
   *     ResourceRoleRepository}
   */
  @Transactional
  public AiKnowledgeDocument upload(
      String sourceUri, String sourceKind, Collection<String> allowedRoles) {
    return upload(sourceUri, sourceKind, allowedRoles, null);
  }

  /**
   * Extended upload that captures an optional Jmix entity link ({@code sourceEntityName}) so it is
   * persisted on the document BEFORE {@code dataManager.save()} runs and BEFORE the async ingester
   * schedules. Required by Plan 10-08 / Phase 10 D-07: chunk metadata ({@link
   * com.vn.agent.rag.ChunkMetadata#SOURCE_ENTITY}) is mirrored from the document row at ingest
   * time, so the field MUST already be persisted when {@link
   * AsyncIngestionWorker#ingest(java.util.UUID)} reloads the row.
   *
   * @param sourceUri Spring {@code ResourceLoader}-resolvable URI
   * @param sourceKind free-form label (MIME type or synthetic marker); nullable
   * @param allowedRoles Jmix role codes granting retrieval visibility; nullable (treated as empty)
   * @param sourceEntityName Jmix entity name (MetaClass#getName) to link the document to; nullable
   *     (no link → chunks ingest without {@code source_entity} metadata; legacy-doc contract per
   *     D-06)
   */
  @Transactional
  public AiKnowledgeDocument upload(
    ...
    List<String> roles =
        KnowledgeDocumentRoleValidator.validateRoleCodes(allowedRoles, roleRepository);
    ...
    document.setCreatedBy(resolveCurrentUsername());
    document.setSizeBytes(resolveSizeBytes(normalizedSourceUri));
    ...
    if (TransactionSynchronizationManager.isSynchronizationActive()) {
      ...
      log.warn(
          "upload: no active transaction synchronization; dispatching ingest inline for {}",
          saved.getId());
      asyncIngestionWorker.ingest(saved.getId());
    }
  ...
  /**
   * CR-01 allowlist validator. Only {@code classpath:} URIs matching a configured prefix and {@code
   * file:} URIs resolving inside the configured staging root are accepted. Anything else —
   * including raw {@code file:} with no staging root configured, other schemes, or classpath
   * resources outside the allowlist — is rejected as an {@link IllegalArgumentException}. Bare
   * filenames (no scheme) are treated as an implicit {@code classpath:ai-kb/<name>} which must also
   * fall inside the allowlist to succeed.
   */
  private String validateSourceUri(String sourceUri) {
    ...
    if (trimmed.startsWith(FILE_PREFIX)) {
      ...
      if (!isUnder(staging, target)) {
        throw new IllegalArgumentException(
            "file: sourceUri must resolve inside configured staging root ("
                + stagingRoot
                + "): "
                + trimmed);
      }
  ...
  private void validateClasspathUri(String location) {
    if (containsPathTraversal(location)) {
      throw new IllegalArgumentException(
          "classpath sourceUri must not contain path traversal: " + location);
    }
    ...
    boolean allowed =
        prefixes.stream().map(StringUtils::cleanPath).anyMatch(normalized::startsWith);
    if (!allowed) {
      throw new IllegalArgumentException(
          "sourceUri classpath location is not allowed: "
              + location
              + " (allowed prefixes: "
              + prefixes
              + ")");
  ...
  private String resolveCurrentUsername() {
    try {
      UserDetails user = currentAuthentication == null ? null : currentAuthentication.getUser();
  ...
  private Long resolveSizeBytes(String normalizedSourceUri) {
    if (resourceLoader == null || normalizedSourceUri == null || normalizedSourceUri.isBlank()) {
      return null;
    }
    String resourceLocation = normalizedSourceUri;
    if (!resourceLocation.startsWith(CLASSPATH_PREFIX)
        && !resourceLocation.startsWith(FILE_PREFIX)
        && !resourceLocation.contains("://")) {
      resourceLocation = "classpath:ai-kb/" + resourceLocation;
    }
    try {
      ...
      log.debug("Could not resolve knowledge document size for {}", normalizedSourceUri, ex);
      return null;
    }
  }
  ...
  private static boolean containsPathTraversal(String location) {
    String path =
        location.startsWith(CLASSPATH_PREFIX)
  ...
  private static String writeRolesJson(List<String> roles) {
    try {
      ...
      // List<String> is trivially serializable; an exception here is a JVM-level bug, not a caller
      // problem.
