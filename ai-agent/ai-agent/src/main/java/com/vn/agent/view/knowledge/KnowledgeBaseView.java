 ...
 * Uploads are staged into Jmix {@link TemporaryStorage} via Vaadin's non-deprecated {@link
 * UploadHandler#toFile(com.vaadin.flow.server.streams.FileUploadCallback,
 * com.vaadin.flow.server.streams.FileFactory)} path — Jmix's {@code receiverType} uses {@code
 * Upload.setReceiver(...)} which is {@code @Deprecated(since="24.8", forRemoval=true)}, so we stick
 * with the UploadHandler callback. Each file lands under the temp root validated by {@link
 * KnowledgeDocumentUploadService#upload(String, String, java.util.Collection)} before async
 * ingestion begins. Only {@link FileRejectedEvent} is wired declaratively via {@code @Subscribe}
 * because it does not require {@code getReceiver()}.
 *
 * <p><b>Size cap:</b> {@code jmix.ai-agent.rag.upload.max-file-size-bytes} clamps client-side
 * uploads; the host application should keep {@code spring.servlet.multipart.max-file-size} and
 * {@code spring.servlet.multipart.max-request-size} aligned above it. The server {@code
 * KnowledgeDocumentUploadService} also enforces a character cap during async ingestion via {@code
 * jmix.ai-agent.rag.ingest.max-document-chars}. For the {@code file:} URI path to succeed, the host
 * application MUST set {@code jmix.ai-agent.rag.upload.file-staging-root} to the Jmix temp directory
 * (typically {@code ${jmix.core.work-dir}/temp}). The view logs a WARN and rejects uploads if the
 * root is not configured (Rule 2 — safe default, fail-closed).
 *
 * <p><b>Push refresh (D-16):</b> Spring {@code ApplicationListener} receives {@link
 * DocumentStatusChangedEvent} (published by {@code IngestionStatusWriter} in 07-02 via {@code
 * afterCommit}) and updates the affected row via {@code UI.access()}. Per-UI dispatch — the ownerUi
 * reference is captured onAttach and cleared onDetach.
 *
 * <p><b>Row actions (D-18):</b> Reingest + Delete both open confirmation dialogs backed by {@link
...
public class KnowledgeBaseView extends StandardListView<AiKnowledgeDocument> {
  ...
  private static final DateTimeFormatter UPLOADED_AT_FORMATTER =
  ...
  @ViewComponent private DataGrid<AiKnowledgeDocument> documentsDataGrid;
  @ViewComponent private JmixUpload documentUpload;
  @ViewComponent private CollectionLoader<AiKnowledgeDocument> documentsDl;
  @ViewComponent private CollectionContainer<AiKnowledgeDocument> documentsDc;
  @ViewComponent private JmixComboBox<MetaClass> sourceEntityNameComboBox;
  @ViewComponent private JmixMultiSelectComboBox<String> uploadAllowedRolesField;

  @Autowired private KnowledgeDocumentUploadService uploadService;
  ...
  @Autowired private TemporaryStorage temporaryStorage;
  ...
  public void onInit(final InitEvent event) {
    ...
    sourceEntityNameComboBox.setItemLabelGenerator(
        mc -> messageTools.getEntityCaption(mc) + " (" + mc.getName() + ")");

    // WARNING-08: populate the upload-form allowedRoles selector with the same
    ...
    List<String> uploadRoleCodes =
        resourceRoleRepository.getAllRoles().stream()
            .map(ResourceRole::getCode)
            .filter(KnowledgeBaseView::isUserSelectableRole)
        ...
        UploadHandler.toFile(
            (metadata, stagedFile) -> {
              try {
                ...
                Set<String> selectedRoles = uploadAllowedRolesField.getValue();
                Collection<String> roles =
                    selectedRoles == null
                        ? Collections.emptySet()
                        : new LinkedHashSet<>(selectedRoles);
                uploadService.upload(
                    stagedFile.toURI().toString(), metadata.contentType(), roles, sourceEntityName);
                notifications
                    .create(
                        messages.formatMessage(
                            "knowledgeBase.toast.uploadStarted", metadata.fileName()))
                    .show();
                documentsDl.load();
  ...
  public void onDocumentsDlPostLoad(
      final CollectionLoader.PostLoadEvent<AiKnowledgeDocument> event) {
  ...
  private Renderer<AiKnowledgeDocument> documentsDataGridActionsRenderer() {
    return DataGridRenderers.buildActionsColumn(
        ...
        new LinkedHashSet<>(
            List.of(
                ActionColumnType.VIEW,
                ActionColumnType.EDIT,
                ActionColumnType.RELOAD,
                ActionColumnType.DELETE)),
        (row, type) -> {
          documentsDataGrid.select(row);
          switch (type) {
            case VIEW -> navigateToDocumentDetail(row);
            case EDIT -> onEditPermissionsClick();
  ...
  /**
   * Renders the source-entity column. Shows the localized entity caption with the raw {@code
   * MetaClass.getName()} in parentheses (matching the ComboBox label format), or an em-dash
   * placeholder when the document has no link (Plan 10-08, D-06 legacy-doc contract).
   */
    ...
    return new ComponentRenderer<>(
        doc -> {
          ...
          if (name == null || name.isBlank()) {
            ...
            span.setText(mc != null ? messageTools.getEntityCaption(mc) + " (" + name + ")" : name);
          }
          return span;
        });
  }

  @Supply(to = "documentsDataGrid.uploadedDate", subject = "renderer")
  private Renderer<AiKnowledgeDocument> documentsDataGridUploadedDateRenderer() {
    return new TextRenderer<>(
        doc -> {
          OffsetDateTime uploadedAt =
              doc.getCreatedDate() != null ? doc.getCreatedDate() : doc.getIngestedAt();
          return uploadedAt == null ? "" : UPLOADED_AT_FORMATTER.format(uploadedAt);
        });
  }
  ...
  /**
   * Push refresh listener (D-16). Runs on the Spring event thread; hops onto the per-tab UI via
   * {@code UI.access()} before touching the grid. If the view is detached (ownerUi is null) the
   * event is silently dropped — the next list reload will pick up the row.
   */
  @EventListener
  public void onDocumentStatusChanged(DocumentStatusChangedEvent event) {
    ...
    ui.access(
        () -> {
          ...
          doc.setErrorMessage(
              event.status() == AiKnowledgeDocumentStatus.FAILED ? event.errorMessage() : null);
          documentsDc.replaceItem(doc);
        });
  }

  /**
   * Jmix-native deep-link hook. Fires after {@code InitEvent} on navigation-opened views — see Jmix
   * docs: Flow UI → Views → View Events → QueryParametersChangeEvent. The captured id is applied
   * after the loader finishes via {@link #selectPendingDocumentIfNeeded()}.
   */
  @Subscribe
  ...
  private void navigateToDocumentDetail(AiKnowledgeDocument doc) {
    if (doc == null || doc.getId() == null) {
      return;
    }
    viewNavigators
        .view(this, KnowledgeDocumentDetailView.class)
        .withQueryParameters(QueryParameters.simple(Map.of("documentId", doc.getId().toString())))
        .navigate();
  }
  ...
  private void onReingestClick() {
    ...
    dialogs
        .createOptionDialog()
        .withHeader(
            messages.formatMessage("knowledgeBase.confirm.reingest.title", doc.getFileName()))
                ...
                .withHandler(
                    ev -> {
                      try {
                        ...
                        NotificationUtils.errorWithDetail(
                            notifications, messages, "knowledgeBase.reingest.failed", ex);
                      }
                    }),
  ...
  private void onEditPermissionsClick() {
    ...
    JmixMultiSelectComboBox<String> rolesGroup = createStringMultiSelectComboBox();
    rolesGroup.setLabel(messages.getMessage("knowledgeBase.upload.field.allowedRoles"));
    rolesGroup.setHelperText(messages.getMessage("knowledgeBase.upload.field.allowedRoles.helper"));
    rolesGroup.setPlaceholder(
        messages.getMessage("knowledgeBase.upload.field.allowedRoles.placeholder"));
    rolesGroup.setWidthFull();
    List<String> allRoleCodes =
        resourceRoleRepository.getAllRoles().stream()
            .map(ResourceRole::getCode)
            .filter(KnowledgeBaseView::isUserSelectableRole)
            .sorted(Comparator.naturalOrder())
    ...
    JmixComboBox<MetaClass> entityCombo = createMetaClassComboBox();
    entityCombo.setLabel(messages.getMessage("knowledgeBase.upload.field.sourceEntityName"));
    entityCombo.setHelperText(
        messages.getMessage("knowledgeBase.upload.field.sourceEntityName.helper"));
    entityCombo.setItems(metaclassComboBoxHelper.buildFilteredList());
    entityCombo.setItemLabelGenerator(
        mc -> messageTools.getEntityCaption(mc) + " (" + mc.getName() + ")");
        ...
        new Button(
            ...
            e ->
                confirmAndSavePermissions(
                    doc, rolesGroup.getValue(), entityCombo.getValue(), dialog));
  ...
  /**
   * Reingest-required confirmation step (UI-SPEC: Surface 3, copywriting table). On OK delegates to
   * the service; the view does not orchestrate save+reingest itself (Fix W3, CLAUDE.md "no business
   * logic in views").
   */
  private void confirmAndSavePermissions(
      AiKnowledgeDocument doc, Set<String> selectedRoles, MetaClass selectedEntity, Dialog dialog) {
    dialogs
        .createOptionDialog()
        .withHeader(messages.getMessage("knowledgeBase.confirm.editPermissions.reingest.title"))
        .withText(messages.getMessage("knowledgeBase.confirm.editPermissions.reingest.body"))
        .withActions(
            new DialogAction(DialogAction.Type.OK)
                .withVariant(ActionVariant.PRIMARY)
                .withHandler(
                    ev -> {
                      ...
                      try {
                        UpdatePermissionsResult result =
                            documentService.updatePermissionsAndReingest(
                                doc.getId(), roles, sourceEntityName);

                        if (result.status()
                            == UpdatePermissionsResult.Status.SAVED_AND_REINGESTING) {
                          notifications
                              .create(
                                  messages.getMessage("knowledgeBase.notification.reingestStarted"))
                              .show();
                        }
                      } catch (Exception ex) {
  ...
  /**
   * WARNING-02: Jmix system roles (system-full-access, system-minimal, ...) are not intended
   * targets for "end-user can read this knowledge document". Filter them out of the role-selection
   * UI so admins only see project-supplied resource roles.
   */
  private static boolean isUserSelectableRole(String code) {
    return code != null && !code.startsWith("system-");
  }

  @SuppressWarnings("unchecked")
  private JmixMultiSelectComboBox<String> createStringMultiSelectComboBox() {
    return uiComponents.create(JmixMultiSelectComboBox.class);
  }

  @SuppressWarnings("unchecked")
  private JmixComboBox<MetaClass> createMetaClassComboBox() {
    return uiComponents.create(JmixComboBox.class);
  }
                ...
                .withHandler(
                    ev -> {
                      try {
                        ...
                        NotificationUtils.errorWithDetail(
                            notifications, messages, "knowledgeBase.delete.failed", ex);
                      }
                    }),
            new DialogAction(DialogAction.Type.CANCEL))
        .open();
  ...
  private void notifyError(String message) {
    notifications.create(message).withThemeVariant(NotificationVariant.LUMO_ERROR).show();
  }
}
